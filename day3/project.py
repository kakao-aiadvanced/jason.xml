...
def grade_documents(state):
    ...
    grade_documents_retry = state["grade_documents_retry"]

    return {"documents": filtered_docs, "question": question, "web_search": web_search, "grade_documents_retry": grade_documents_retry + 1 }
...
def web_search(state):
    ...
    # Web search
    docs = tavily_client.search(query=question, max_results=3)['results']
    ...
    hallucination_retry = state["hallucination_retry"]
    return {"documents": documents, "question": question, "hallucination_retry": hallucination_retry + 1}
...
def grade_generation_v_documents_and_question(state):
    ...
    if grade == "yes":
        ...
        if state["hallucination_retry"] == 0:
            pprint("---DECISION: GENERATION IS NOT GROUNDED IN DOCUMENTS, RE-TRY---")
            return "not supported"
        else :
            pprint("---DECISION: GENERATION HAS HALLUCINATION, STOP-PROCESS---")
...
def run_workflow() :
    ...
    # Test
    inputs = {"question": "What is prompt?", "hallucination_retry": 0}
...
if __name__ == '__main__':
    ...
    hallucination_grader = get_retrieval_hallucination_grader()
    ...
    # ret = retrieve(state)
    # # TODO 이건 노드 테스트용.
    # state["question"] = ret['question']
    # state['documents'] = ret['documents']
    # generate(state)
